...
Feature: Compute fontSize on a single EBU-TT Live element
  ...
  Scenario Outline: Font size inheritance
    Given an xml file "ebuttd_fontsize_convert.xml"
    And it has the following template variables
      | cell_resolution | style_attribute | extent |
      | 32 15           | tts:fontSize    |        |
    ...
    And the document is converted to EBUTTD with local time mapping "00:00:00"
