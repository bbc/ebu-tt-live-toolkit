...
@when(**legacy_name(name='it has frameRate <frame_rate>'))
def when_frame_rate(frame_rate, template_dict):
    template_dict['frame_rate'] = frame_rate


@when(**legacy_name(name='it has frameRateMultiplier <frame_rate_multiplier>'))
...
@when(**legacy_name(name='it has dropMode <drop_mode>'))
def when_drop_mode(drop_mode, template_dict):
    template_dict['drop_mode'] = drop_mode


@when(**legacy_name(name='it has markerMode <marker_mode>'))
