...
class TimingValidationMixin(object):
    ...
    semantic validation for timing attributes in the document's timeBase.
    """
    ...
    def _pre_timing_set_attribute(self, attr_en, attr_use):
        # Pass in the timing_attribute_name to the context to help the timing
        # type constructor refuse creation
    ...
    def _pre_calculate_end(self):
        ...
        if self._end_timedelta is not None:
            if self._semantic_dataset['timing_end_stack']:
                ...
                proposed_end = min(
                    self._semantic_dataset['timing_syncbase'] + self._end_timedelta,
                    self._semantic_dataset['timing_end_stack'][-1])
            # New end
            else:
                proposed_end = \
                    self._semantic_dataset['timing_syncbase'] + self._end_timedelta
    ...
    def _pre_assign_begin(self, proposed_begin):
        ...
        # If we have a non-zero availability time we need to factor it in
        # BUT the syncbase stays
        if self._semantic_dataset['availability_time']:
            self._computed_begin_time = max(
                self._semantic_dataset['timing_syncbase'],
    ...
    def _post_calculate_begin(self, children):
        """
        The computed begin time shall be moved down to match that of the
        earliest child begin time in case the container does not specify a
        begin time itself.

        NOTE: This does not modify the syncbase.
        ...
        children_computed_begin_times = \
            [item.computed_begin_time for item in children]
    ...
    def _semantic_preprocess_timing(self, dataset, element_content):
        """
        As the validator traverses in a Depth First Search this is the hook
        function to call on the way DOWN.
    ...
    def _post_pop_begin(self):
        ...
        if self._begin_timedelta is not None:
            ...
            # It could probably be removed and replaced
            # with self._begin_timedelta
    ...
    def _semantic_postprocess_timing(self, dataset, element_content):
        """
        As the validator traverses in a Depth First Search this is the hook
        function to call on the way UP
        ...
        Steps to take:
          ...
          - Finalize computed_begin_time if begin is not specified using
            computed_begin_time of children.
        ...
        # If the forward running part of the traversal could not assign an
        # end time we can use the backwards route which is in a way similar
        # to dynamic programming because we take the children computed times
        # and take the maximum value from them.
        # SPECIAL case: a single leaf element with an undefined end time
        # renders the entire branch undefined
        ...
        if end_timedelta is not None and self.computed_end_time is None \
            ...
            # Should the calculation be changed this filters out an obvious
            # source of error.
            raise LogicError()
        ...
        if self.computed_end_time is None:
            ...
            # All timing containers are complexTypes so we can call
            # orderedContent safely but we don't want to bother with
            # explicitly badly timed elements so filter them out.
            # children = [
            #     child.value for child in self.orderedContent()
            #     if isinstance(child.value, TimingValidationMixin)
            #     and not child.value._element_badly_timed(value=None, element=child)
            #     ]
            children = [
                item for item in [x.value for x in self.orderedContent()]
                if isinstance(item, TimingValidationMixin)
            ...
            if not children:
                ...
                if not self._semantic_dataset['timing_end_stack']:
                    # Here we go an endless document. Pointless but for
                    # clarity's sake assign it explicitly to None.
                    self._computed_end_time = None
                else:
                ...
                children_computed_end_times = \
                    [item.computed_end_time for item in children]
        ...
        # When we are the body element we need to check that our explicit
        # timings are valid, i.e. deal with end before befin by discarding
        # this element from computed time calculation as per spec requirement.
        # Since we exclude all elements where this is the case using the
        # _element_badly_timed function as a filter, this only applies to the
        # body element (on which the filter function doesn't get called).
    ...
    # take part in the traversal directly we process them in the timed
    # element's context instead: body, div, p, span
...
class BodyTimingValidationMixin(TimingValidationMixin):
    """
    The body element seems to be exception from too many rules and makes one
    common validator pretty difficult to manage. This subclass is meant to
    call all the extensions/limitations for the body element that does not
    apply to timed containers in general in the EBU-TT-Live spec.
    """
    ...
    def _pre_calculate_end(self):
        ...
        if self._dur_timedelta is not None:
            if self._begin_timedelta is not None \
               and self._end_timedelta is not None:
                # This is a special (stupid) edge case..:
                proposed_end = min(
                    self._dur_timedelta + self._begin_timedelta,
                    self._end_timedelta)
            elif self._begin_timedelta is not None \
                    and self._end_timedelta is None:
                proposed_end = self._dur_timedelta + self._begin_timedelta
            elif self._begin_timedelta is None \
                    and self._end_timedelta is None:
                # In this case the document end at availability time + dur
                proposed_end = \
                    self._semantic_dataset['availability_time'] + self._dur_timedelta
            elif self._begin_timedelta is None \
                    and self._end_timedelta is not None:
                proposed_end = min(
                    self._semantic_dataset['availability_time'] + self._dur_timedelta,
