...
@when(parsers.parse(name='it has timeBase {time_base}'))
def when_time_base(time_base, template_dict):
    template_dict['time_base'] = time_base.strip('"')


@when(**legacy_name(name='it has sequenceIdentifier <sequence_identifier>'))
@when(parsers.parse(name='it has sequenceIdentifier {sequence_identifier}'))
def when_seq_id(sequence_identifier, template_dict):
    template_dict['sequence_identifier'] = sequence_identifier.strip('"')
...
def when_doc1_added_to_seq(doc1_avail_time, template_file, template_dict, sequence, test_context):
    ...
    doc1_avail_time = doc1_avail_time.strip('"')
...
@when(**legacy_name(name='doc2 is added to the sequence with availability time <doc2_avail_time>'))
@when(name=parsers.parse(
    name='doc2 is added to the sequence with availability time {doc2_avail_time}'))
def when_doc2_added_to_seq(doc2_avail_time, template_file, template_dict, sequence, test_context):
    xml_file = template_file.render(template_dict)
    # print('doc2 xml_file:')
    # print(xml_file)
    document = EBUTT3Document.create_from_xml(xml_file)
    doc2_avail_time = doc2_avail_time.strip('"')
...
@when(**legacy_name(name='doc3 is added to the sequence with availability time <doc3_avail_time>'))
def when_doc3_added_to_seq(doc3_avail_time, template_file, template_dict, sequence, test_context):
    xml_file = template_file.render(template_dict)
    # print('doc3 xml_file:')
    # print(xml_file)
    document = EBUTT3Document.create_from_xml(xml_file)
    doc3_avail_time = doc3_avail_time.strip('"')
...
@then(**legacy_name(name='doc1 has resolved begin time <r_begin_doc1>'))
...
@then(**legacy_name(name='doc1 has resolved end time <r_end_doc1>'))
...
@then(**legacy_name(name='doc2 has resolved begin time <r_begin_doc2>'))
...
@then(**legacy_name(name='doc2 has resolved end time <r_end_doc2>'))
...
@then(**legacy_name(name='doc3 has resolved begin time <r_begin_doc3>'))
...
@then(**legacy_name(name='doc3 has resolved end time <r_end_doc3>'))
