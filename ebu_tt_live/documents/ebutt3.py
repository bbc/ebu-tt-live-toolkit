...
from ebu_tt_live.strings import (
    ...
    DOC_INSERTED,
    DOC_REQ_SEGMENT,
    DOC_SEMANTIC_VALIDATION_SUCCESSFUL,
    DOC_SEQ_REQ_SEGMENT,
    DOC_TRIMMED,
...
class TimingEvent:
    """
    This class wraps a document and an associated resolved timing event into
    an object that can be placed on the timeline.
    """
...
class TimingEventBegin(TimingEvent):
    ...
    def __init__(self, element):
        super().__init__(element=element, when=element.computed_begin_time)

    def __repr__(self):
        return f'<{type(self)}({self.when}): {self.element}>'
...
class TimingEventEnd(TimingEvent):
    ...
    """
    def __init__(self, element):
        super().__init__(element=element, when=element.computed_end_time)

    def __repr__(self):
        return f'<{type(self)}({self.when}): {self.element}>'


class TimelineUtilMixin:
    """
    This mixin is responsible for managing the shared timeline functionality
    """

    # The timing events that mark the beginning and end of an element are kept
    # on a timeline, which is a sorted list.
    # IMPORTANT: Not sorted set as there are overlapping begins and ends.
    _timeline = None

    @property
    def timeline(self):
        if self._timeline is None:
            self._timeline = sortedlist.SortedListWithKey(
    ...
    def lookup_range_on_timeline(self, begin=None, end=None):
        ...
        for item in self.timeline.irange(
                maximum=end is not None and TimingEvent(None, end) or None):
            ...
            if isinstance(item, TimingEventBegin):
                ...
                if begin is not None and item.when <= begin:  # noqa: SIM102
                    # Remove elements, which had ended before the specified
                    # range began.
...
class EBUTT3ObjectBase:

    message_type_mapping: ClassVar = {}
    ...
    def create_from_xml(cls, xml, **kwargs):
        ...
        if isinstance(instance, ebuttlm.message_type):
            return \
                cls.message_type_mapping[instance.header.type] \
                .create_from_raw_binding(instance)
...
class EBUTTAuthorsGroupControlRequest(EBUTTLiveMessage):
    ...
    def create_from_raw_binding(
        ...
        return cls(
            ...
            payload=binding.payload.orderedContent()[0].value  # anyType is 
            # considered complex type so orderedContent is needed
            # and indexing the first component.
        )


# Register the class in the base class
EBUTT3ObjectBase.message_type_mapping[
    EBUTTAuthorsGroupControlRequest.message_type_id] = \
        EBUTTAuthorsGroupControlRequest


class EBUTT3Document(TimelineUtilMixin, SubtitleDocument, EBUTT3ObjectBase):
    """
    This class wraps the binding object representation of the XML and provides
    the features the applications in the specification require. 
    e.g: availability time.
    """
    ...
    # a very basic hash function - the sortedset key function will
    # define the actual sort key.
    # The __eq__ function is already provided by ComparableMixin
    def __hash__(self):
        return hash(id(self))
    ...
    @classmethod
    def create_from_raw_binding(cls, binding, availability_time=None):  # ty:ignore[invalid-method-override]
    ...
    @classmethod
    def create_from_xml(cls, xml, availability_time=None):  # ty:ignore[invalid-method-override]
    ...
    @property
    def sequence_identifier(self):
        if not self._ebutt3_content:
            raise ValueError(ERR_DOCUMENT_NO_TT_CONTENT)
    ...
    def discard_document(self, resolved_end_time, verbose=False):
        ...
        if verbose:
            document_logger.info(
                f'Document discarded:\n'
                f'{self.content_to_string(end=resolved_end_time)}'
            )
    ...
    def content_to_string(self, begin=None, end=None):
        """
        Extract the document textual content between begin/end along with
        their activation times.

        :param begin: From specified begin time or resolved begin time if
                      begin is not specified.
        :param end: Up to specified end time or resolved end time if end is
                    not specified.
        ...
        if end is not None and (
                (self.resolved_end_time is not None
                    and end < self.resolved_end_time)
                or self.resolved_end_time is None):
            resolved_end = end
        else:
    ...
    def validate(self):
        ...
        # Reset timeline
        self.reset_timeline()
        # This is assuming availability from the beginning of our time
        # coordinate system.
        ...
        # Default value for the computed begin time of the document is the
        # active begin time of the body
    ...
    def extract_segment(self, begin=None, end=None, deconflict_ids=False):
        """
        Create a valid ebutt3 document subset. As it collects data it will
        also prefix the ids in the document with the document sequence
        number so that later merge does not have collision.

        :param begin:
        :param end:
        :param deconflict_ids: prevent id clash across documents by prefixing
                               the IDs
        ...
        segmenter = EBUTT3Segmenter(
            document=self,
            begin=begin,
            end=end,
            deconflict_ids=deconflict_ids)
        return EBUTT3Document.create_from_raw_binding(segmenter.segment)

    def cleanup(self):
        """
        This function is meant to get rid of all the validation added data
        that may be blocking garbage collection of the objects.
...
class EBUTT3DocumentSequence(TimelineUtilMixin, CloningDocumentSequence):
    """
    EBU-TT Live specific document sequence. It maps the documents based on
    their sequence numbers and timing attributes.

    The sequence object can be used in 2 different modes:
      - It can be used to produce a sequence(i.e.: new_document method)
      - as well as it is the pivotal point of the consumer use-case when the
        document timings need to be resolved.
        (i.e.: add_document method)

    The sequence is responsible to keep the documents ordered and filter those
    documents out, which were eventually overwritten. It ensures that at any
    given time exactly 0 or 1 document is active (R14).
    """
    ...
    def __init__(
        ...
        # The documents are kept in a sorted set that is sorted by the
        # documents's sequence number
    ...
    def create_from_document(cls, document, verbose=False, *args, **kwargs):
        ...
        return cls(
            sequence_identifier=kwargs.get(
                'sequence_identifier',
                document.sequence_identifier),
            authors_group_identifier=kwargs.get(
                'authors_group_identifier',
                document.authors_group_identifier),
            reference_clock=kwargs.get(
                'reference_clock',
                get_clock_from_document(document)),
            lang=kwargs.get('lang', document.lang),
            verbose=verbose
        )

    def _check_document_compatibility(self, document):
        if not self._reference_clock:
            raise ValueError(ERR_DOCUMENT_SEQUENCE_NO_REF_CLOCK)
        if self.sequence_identifier != document.sequence_identifier or \
            ...
            raise IncompatibleSequenceError(
                ERR_DOCUMENT_NOT_COMPATIBLE.format(
                    'ebuttp:sequenceIdentifier or ttp:timeBase')
            )
        if self._reference_clock.time_base == 'clock' and \
           self._reference_clock.clock_mode != document.clock_mode:
            raise IncompatibleSequenceError(
                ERR_DOCUMENT_NOT_COMPATIBLE.format('ttp:clockMode')
            )
        existing_document = None
        if self._documents is None:
            raise ValueError(ERR_DOCUMENT_SEQUENCE_NO_DOCSET)
        try:
            # Locate the insertion position
            insertion_idx = self._documents.bisect_key_left(
        ...
        if existing_document is not None:
            ...
            # If there is an authorsGroupIdentifier and sequence does not have
            # one yet. Pick it up.
            # If there is one already mismatch results in an error but missing
            # value is allowed even then.
            if document.authors_group_identifier is not None \
               and document.authors_group_identifier != "":
                if self.authors_group_identifier is not None:
                    if document.authors_group_identifier != \
                       self.authors_group_identifier:
                    ...
                    # Set the one we currently have in the document except the
                    # empty string
                    self._authors_group_identifier = \
                        document.authors_group_identifier

            log.debug(
                f'Sequence number: {document.sequence_number} can be '
                f'safely inserted into sequence: {self.sequence_identifier}')
        return True
    ...
    def create_compatible_document(self, *args, **kwargs):
        """
        This utility function is used by the converter to extract segments and
        by the new_document function.
    ...
    def _insert_or_discard(self, document):
        """
        This function does the heavy lifting of timing resolution. It inspects
        the close vicinity in which the
        ...
        Based on that information it can detect 3 different cases:
          - out of order delivery of documents in which case it inserts and
            trims the document,
          - sequence override scenario in which case an exception is raised.
          - Document discard scenario in which case the document is already
            overwritten in the sequence so it will not get inserted.

        :raises: ValueError, SequenceOverridden, DocumentDiscardedError
        """
        # Our document's begin and end times. Initially they correspond with
        # the computed document begin- and end times.
        ...
        for item in self.timeline.irange(
            ...
            if item.element.sequence_number > document.sequence_number:
                ...
                discarderror = DocumentDiscardedError(item.element)
                raise discarderror
        ...
        for item in self.timeline.irange(this_begins):
            ...
            if isinstance(item, TimingEventEnd):
                ends_after = item
                if begins_before \
                   and ends_after.element != begins_before.element \
                   and ends_after.when != begins_before.when:
                    raise ValueError(ERR_DOCUMENT_SEQUENCE_INCONSISTENCY)
        ...
        if begins_before:
            ...
            if self._verbose:
                document_logger.info(
                    f'Document trimmed by next one:\n'
                    f'{begins_before.element.content_to_string()}'
                )
    ...
    def _insert_document(self, document, ends=None):
        ...
        if self._verbose:
            document_logger.info(
                f'New document inserted:\n{document.content_to_string()}'
            )

    def _override_sequence(self, document):
        """
        This function clears the timeline and the associated documents after
        the document in the parameter.
        This happens when a document with a higher sequence number is added
        preceding some other documents with lower sequence numbers.
        ...
        for item in self.timeline.irange(resolved_begin):
            if item.element.sequence_number < sequence_number:
                if isinstance(item, TimingEventEnd) \
                   and item.element not in discarded_timing_events:
                    # We found the end event of a document whose begin event
                    # was not encountered. Meaning that instead
                    # of discarding it we are supposed to trim it. R17
                    continue
                else:
                    discarded_timing_events.setdefault(item.element, []) \
                        .append(item)
    ...
    def extract_segment(
            ...
            discard=False):
        """
        Extract the subtitles from the sequence in the given timeframe. The
        return value is one merged EBUTT3Document
        ...
        affected_documents = self.lookup_range_on_timeline(
            begin=begin,
            end=end)
        ...
        for doc in affected_documents:
            ...
            if end is not None and (doc_ending is None or end < doc_ending):
                doc_ending = end
            # Check only til resolved end, otherwise there will be unwanted
            # parallel elements
            try:
                doc_segment = doc.extract_segment(
                    begin=begin,
                    end=doc_ending,
                    deconflict_ids=True)
                ...
                log.error(
                    f'Error extracting document segment from '
                    f'{doc.sequence_identifier}__{doc.sequence_number}'
                )
                log.error(repr(err))
        ...
        if not document_segments:
            # TODO: This is good question what now? no documents found for
            # range
            ...
            splicer = EBUTT3Splicer(
                sequence_identifier=f'{self.sequence_identifier}_resegmented',
                sequence_number=current_sequence_number,
                document_segments=document_segments
            )
            document = EBUTT3Document.create_from_raw_binding(
                splicer.spliced_document)
